...
from watchlist.tasks import compile_stock_qualitative, compile_stock_quantitative
...
def test_publish_sends_a_workspace_event_to_the_workspace_group(workspace):
    ...
    with patch("common.events.get_channel_layer", return_value=Layer()):
...
def test_publish_is_best_effort_without_a_channel_layer(workspace):
    with patch("common.events.get_channel_layer", return_value=None):
...
def test_stock_page_compiles_publish_one_event_per_measure(workspace):
    ...
    with patch("watchlist.tasks.publish") as publish:
...
def test_every_evaluation_outcome_publishes_strategy_evaluated(workspace):
    ...
    with patch("strategies.tasks.publish") as publish:
