...
And the honest engineering, right there in the code: this only works when there's an LLM to call. With no API key, the system doesn't pretend. Open [`backend/ai/claude_client.py`](../backend/ai/claude_client.py) — the very first thing `ClaudeClient.assess` does:
