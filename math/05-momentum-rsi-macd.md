...
Open [`backend/markets/indicators.py`](../backend/markets/indicators.py). Here is the RSI, matching §5.2 step for step — the gains/losses split, the `rsi_val` rescaling with its `avg_loss == 0 → 100` short-circuit, the seed average over the first `period` changes, and the Wilder recurrence:
