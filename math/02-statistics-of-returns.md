...
5. **In the code.** Read `_zscore_series` in [`indicators.py`](../backend/markets/indicators.py). Feed it the six closes `[101, 100, 102, 100, 101, 90]` with `window = 6` and predict the single non-`None` value it returns *before* running it (you may peek ahead to Chapter 3, or just apply §2.2–2.8 directly). Then change `ddof=1` to `ddof=0` in a scratch copy and recompute: does the magnitude of the z-score go up or down, and why does that match the direction of the bias you found in Problem 1?
