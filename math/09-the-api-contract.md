...
And here is the enforcement, the single choke point every scoped request flows through, in `backend/core/workspaces.py`:
...
A small middleware plucks it out before the connection is accepted, in `backend/strategies/ws_auth.py`:
...
- **Routes** — `backend/config/urls.py` (the base-path table above), `backend/core/urls.py` (a DRF router for `workspaces` and `watchlist`), `backend/strategies/urls.py` (a router for `strategies` and `alerts`, plus explicit `indicators/` and `markets/<ticker>/analysis/` paths).
- **Authorization choke point** — `resolve_active_workspace` in `backend/core/workspaces.py`. Every scoped view calls it; it is the single line where tenancy is enforced over HTTP.
